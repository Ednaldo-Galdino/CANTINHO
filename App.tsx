...
const App: React.FC = () => {
  ...
  const startQuiz = async () => {
    ...
    setLoadingMsg(`Carregando ${selectedAmount} questões de ${selectedSubject}...`);

    try {
      const qs = await questionService.getQuestions(selectedSubject, selectedAmount, selectedDifficulty);
      setQuestions(qs);
      ...
      alert(`Erro ao carregar questões: ${errorMessage}. Tente novamente.`);
    }
  };
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#000814]/90 backdrop-blur-3xl flex flex-col items-center justify-center text-white p-6 text-center">
      ...
      <h2 className="text-xl sm:text-2xl font-black uppercase mb-4 text-cyan-400 italic tracking-widest drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]">Carregando Questões</h2>
